...
4. Two-client first-party 1:1 voice call on web and one
   supported Flutter target (answer/reject/mute/hangup + remote audio)
5. Two-client first-party MatrixRTC/LiveKit group call; Element Call only if JWT/SFU fails
6. Verify TURN on UDP/TCP `3478` and relay ports `49160-49200`; TLS `443`
   belongs to Caddy and is not advertised as TURN/TLS
7. Matrix.org Federation Tester
8. Cron `scripts/backup-highlife.sh`
9. Keep `DEPLOY_PASSWORD` if you use password SSH interactively
